...
export default function DataUpload() {
  ...
  const handleLiveSelection = async (liveName: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/data/get-upload-entry?liveName=${liveName}`
      );
  ...
  const handleDelete = async (uploadId: number) => {
    ...
    try {
      const response = await fetch(
        `/api/data/delete-entry?liveName=${selectedPerformance}&uploadId=${uploadId}`
      );
      const data = await response.json();
        ...
        <Card className="overflow-x-auto shadow-md rounded-lg border border-gray-200 w-full">
          <CardContent>
            <div className="overflow-x-auto w-full">
              <h2 className="text-lg font-medium pb-5 text-center text-white">
                업로드된 파일 목록
              </h2>
              <table className="min-w-full table-auto">
                <thead className="text-white bg-gray-800">
                  <tr>
                    <th className="min-w-[100px] px-4 py-3 text-center text-sm font-medium">
                      기준일자
                    </th>
                    <th className="min-w-[450px] px-4 py-3 text-center text-sm font-medium">
                      파일이름
                    </th>
                  ...
                  {entryData.length > 0 ? (
                    entryData.map((item, index) => (
                      <tr key={index}>
                        ...
                        <td className="min-w-[450px] px-4 py-3 overflow-x-auto text-center">
                          {item.fileName}
                        </td>
                        <td className="min-w-[100px] max-w-[100px] px-4 py-3 overflow-x-auto whitespace-nowrap text-center">
                        ...
                        <td className="min-w-[100px] max-w-[100px] px-4 py-3 text-center text-sm font-medium">
                          <Button
                            variant="outlined"
                            color="primary"
      ...
      {selectedPerformance !== "공연 선택하기" && (
        <div className="w-full flex justify-end">
          <Button
            variant="outlined"
            color="primary"
            onClick={() => setShowDeletedUploads((prev) => !prev)}
          >
